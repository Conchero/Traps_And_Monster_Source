...
public class UITargetArmorNexus : MonoBehaviour
{
    ...
    void Start()
    {
        m_spawnPlayerInvocation = m_UIplayer.m_linkedSpawnPlayerInvocation;
        //Recupère l' ID
        ...
        {
            m_nexus = m_spawnPlayerInvocation.actualTarget.m_linkedNexus.GetComponent<Nexus>();
            m_maxArmor = m_nexus.m_armorMax;
            m_currentArmor = m_nexus.m_armor;
            m_previousArmor = m_currentArmor;
        }
    ...
    private void Update()
    {
        ...
        if (m_playerCount > 1)
        {
            m_nexus = m_spawnPlayerInvocation.actualTarget.m_linkedNexus.GetComponent<Nexus>();
            m_currentArmor = m_nexus.m_armor;
            //if (m_currentArmor != m_previousArmor || m_needUpdate)
            //{
            //    m_needUpdate = false;
            m_progressBar.m_fillAmount = 1f * (float)m_currentArmor / (float)m_maxArmor;
